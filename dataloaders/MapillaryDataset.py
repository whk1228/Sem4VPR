...
class MSLS(Dataset):
    def __init__(self, input_transform = None):
        ...
        self.dbImages = np.load('/root/autodl-tmp/SemVG/dataset/msls_val/msls_val_dbImages_new.npy')

        # hard coded query image names.
        self.qImages = np.load('/root/autodl-tmp/SemVG/dataset/msls_val/msls_val_qImages_new.npy')

        # hard coded index of query images
        self.qIdx = np.load('/root/autodl-tmp/SemVG/dataset/msls_val/msls_val_qIdx.npy')

        # hard coded groundtruth (correspondence between each query and its matches)
        self.pIdx = np.load('/root/autodl-tmp/SemVG/dataset/msls_val/msls_val_pIdx.npy', allow_pickle=True)
