...
def get_semantic_extractor(num_classes=19, output_stride=16):

    return sem.SemNetAtt(num_classes, output_stride)
...
def get_aggregator(agg_arch='ConvAP', agg_config={}):
    ...
    #
    # elif 'sem' in agg_arch.lower():
    #     assert 'num_channels' in agg_config
    #     assert 'num_clusters' in agg_config
    #     assert 'cluster_dim' in agg_config
    #     assert 'token_dim' in agg_config
    #     return aggregators.SemSALAD(**agg_config)
    elif 'addse' in agg_arch.lower():
        assert 'num_channels' in agg_config
        assert 'num_clusters' in agg_config
        assert 'cluster_dim' in agg_config
        assert 'token_dim' in agg_config
        return aggregators.ADDSE(**agg_config)
    else:
        print("Aggregator not found. Returning None")
